...
**Goal:** Implement A4 on `feat/a4`: Strands-based OpenAI-compatible Agent execution, Strands MCP client and Session management, Project multi-session lifecycle, Settings, bounded repair/cancel semantics, Core MCP integration adapters, and the minimal Renderer-facing Agent transport contract. Architecture V1.15 additionally requires the product runtime to converge on one Core-process-scoped MCP connection across Project switches.
...
## Architecture V1.15 Delta — Required Before B1 Project-Switch Integration

The initial A4 implementation used a Project-scoped runtime descriptor and created an MCP client from `projectId` for each invocation. PRD V1.17 / Architecture V1.15 supersede that **formal product topology** while preserving the current nine-tool MCP contract and the standalone `core:mcp --project` development harness.
...
- [ ] **Step 3: Write a failing MCP host test** asserting `tools/list` returns exactly the nine P0 tools and missing/wrong bearer token is rejected.
...
- [ ] **Step 5: Add an integration test against Task 4's real local MCP server** proving Strands sees the nine tools through MCP; V1.15 completion additionally requires Project A close → Project B open with the same MCP endpoint/client and correct active-project guards.
