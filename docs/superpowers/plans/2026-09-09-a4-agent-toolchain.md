...
## Architecture V1.16 Delta — Required Before B1 Project-Switch Integration

The initial A4 implementation used a Project-scoped runtime descriptor and created an MCP client from `projectId` for each invocation. PRD V1.18 / Architecture V1.16 supersede that **formal product topology** while preserving the current ten-tool MCP contract and the standalone `core:mcp --project` development harness.
...
- P0 exposes exactly ten Agent MCP tools: `getTaskContext`, `getScopedComposition`, `submitGenerationPlan`, `requestScopeExtension`, `getOperation`, `cancelOperation`, `replaceScopedMusic`, `updateMusicalProperties`, `resizeComposition`, `finishTask`; operation-creating inputs use caller-stable `operationId`, and Core/MCP injects the Active Project ID for generation plans.
...
- Generation Plan and Scope Extension confirmations are Operation-owned, not Tool-Call-owned. Transport timeout/cancel must not change business state. A4 user/final-failure cancellation explicitly calls `cancelOperation`; Scope Extension cancellation then clears/rejects the matching A3 pending request.
...
- Produces the MCP runtime descriptor contract and runtime validators. Under Architecture V1.15 the formal product descriptor is Core-process-scoped rather than Project-scoped; the older `{projectId,...}` shape is a migration target, not the final B1 contract.
...
- [ ] **Step 3: Write a failing MCP host test** asserting `tools/list` returns exactly the ten P0 tools and missing/wrong bearer token is rejected.
...
**Interfaces:**
- Architecture V1.15 target: runtime descriptor discovery resolves the single live Core instance and rejects stale/malformed descriptors; it does not take `projectId` to choose an endpoint.
...
- [ ] **Architecture V1.15 follow-up:** replace Project-scoped discovery tests with Core-instance discovery tests (live descriptor, dead PID, malformed descriptor, secret-safe errors) and add a Project A → B switch integration proving the same MCP endpoint/client remains usable.
- [ ] **Architecture V1.15 follow-up:** migrate the descriptor reader/factory so Project IDs no longer select endpoints; preserve `projectId` only in workflow/business calls.
...
- [ ] **Architecture V1.15 follow-up:** keep model configuration dynamic per invocation/model call, but move Strands MCP infrastructure construction to the Core-instance lifecycle so Project close/open reuses the same endpoint/client instead of selecting a descriptor per Project.
- [ ] **Step 5: Add an integration test against Task 4's real local MCP server** proving Strands sees the ten tools through MCP; V1.15 completion additionally requires Project A close → Project B open with the same MCP endpoint/client and correct active-project guards.
...
- Spec coverage: Settings ownership, Strands Provider/MCP/Session, Project multi-session, single Core MCP connection across Project switching, dynamic model config, dynamic repair cap, recoverable Operation-based generation-plan confirmation, mechanical confirmed-Task bootstrap, no repair Scope Extension, composition-resize-first long-form generation, recoverable validation diagnostics, unified cancel/final-failure rollback, controlled-fatal cleanup, abrupt-process Core reconciliation, and strict minimal Renderer transport all map to explicit tasks or the V1.16 migration delta above.
