...
import { createAutoApproveConfirmation } from './auto-approve-confirmation.js';
...
describe('createAutoApproveConfirmation', () => {
  it('approves every generation-plan confirmation immediately', async () => {
    const { generationPlan } = createAutoApproveConfirmation();
    await expect(
  ...
  it('approves every scope-extension confirmation immediately', async () => {
    const { scopeExtension } = createAutoApproveConfirmation();
    await expect(
